var compute = function(oneoff, loan, plan, income, p1interest, gradyear) {
  const plan1 = plan == "p1"
  var interest = 0;
  if (plan1) {
    interest = p1interest;
  ...
  const writeoffyears = plan1 ? 25 : 30;

  // Only charged after 17.5k/21k
  ...
  while (loan > 0 && year <= (gradyear + writeoffyears)) {
...
var refreshData = function(loan, plan, p1interest, income, gradyear) {
  var data = []
  for (var oneoff = 0; oneoff <= loan; oneoff += 500) {
    const x = compute(oneoff,  loan, plan, income, p1interest, gradyear);
